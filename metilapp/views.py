...
from .functions import delete_fasta, delete_file, handle_uploaded_file, input_builder, methyl_type_stadistics, patterns_in_genome, read_fasta, read_file_gff, read_gene_gff
...
def main_page(request):
    ...
    if request.method=='POST':
        ...
        if form.is_valid() and validation=="":
            ...
            gene_gff = read_gene_gff('metilapp/'+gene.name)
            ...
            return render(request,'metilapp/result.html', {'message': message, 'patterns': patterns.items(), 'met_name': f.name, 'fasta_name': g.name, 'gene_name': gene.name,
             'jobID': jobID, 'total_m': base_sts[0], 'chrom_m': base_sts[1], 'index_pat': data_fasta[0].items(), 'pat_status': data_fasta[1][0], 'num_st': data_fasta[1][1]})
        else:
            message = form.errors
    form = MetilForm()
